...
true_recf2=true_recf(1:n/2+1,:,:);
t=abs(true_recf2);
true_recf=true_recf(1:length(ome),:,:);
%% run solver
nz=dims.nz;
nx=dims.nx;
...
pf=zeros(dims.nz,dims.nx);
vg=zeros(size(v));
paf=pf;
%% FWI part
tic;
for i=1:length(sx0)
    [pf,recf]=solver(nz,nx,dh,sz0,sx0,ome,sf2,rz,rx,v,lp,d0);
    r=recf-true_recf(:,:,shot_interval*i+1)';
    of_progress(i)=norm(r,2)^2;
    paf=adjoint_solver(nz,nx,dh,sz0,sx0,ome,sf2,rz,rx,v,lp,d0,r);
    for j=2:length(ome)
        vgt=ome(j)^2*diag(pf(:,j))'*paf(:,j);
        vg=vg+reshape(vgt,[size(v,1),size(v,2)]);
    end
end
%%
imagesc(real(vg),.01*[min(real(vg(:))),max(real(vg(:)))])
%%
tic;
for l=1:n_iteration
    ...
    for i=1:length(sx0)
        ...
        n_true_recf=2000;
        fs2=1/dims.dt;
        f2=fs*(0:(n_true_recf/2))/n_true_recf;
        er=fft(ert,n_true_recf,1)/(n_true_recf/2);
        er2=er(1:n_true_recf/2+1,:);
        t=.01*max(abs(er2),[],1);
        ...
        [~,pt2,~,~]=solver(nz,nx,dh,nt,sz2,sx2,ome2(f_range2),n_true_recf,sf2(f_range2,:),rz,rx,v_progress(:,:,l),lp,d0);
        r_pt2=flip(pt2,3);
