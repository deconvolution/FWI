...
for l=2:length(ome)
    ...
    for i=lp+2:nz-1
        for j=lp+2:nx-1
            % i,j
            AA((j-1)*nz+i,(j-1)*nz+i)=ome(l)^2-2*v(i,j)^2/gx(i,j)^2/dh^2-2*v(i,j)^2/gz(i,j)^2/dh^2;
            
            % i,j+1
            AA((j-1)*nz+i,(j)*nz+i)=-v(i,j)^2/2/gx(i,j)^3/dh*gx_x(i,j)+v(i,j)^2/gx(i,j)^2/dh^2;
            % i,j-1
            AA((j-1)*nz+i,(j-2)*nz+i)=v(i,j)^2/2/gx(i,j)^3/dh*gx_x(i,j)+v(i,j)^2/gx(i,j)^2/dh^2;
            
            % i+1,j
            AA((j-1)*nz+i,(j-1)*nz+i+1)=-v(i,j)^2/2/gz(i,j)^3/dh*gz_z(i,j)+v(i,j)^2/gz(i,j)^2/dh^2;
            % i-1,j
            AA((j-1)*nz+i,(j-1)*nz+i-1)=v(i,j)^2/2/gz(i,j)^3/dh*gz_z(i,j)+v(i,j)^2/gz(i,j)^2/dh^2;
        end
    end
