...
true_rec=zeros(dims.nt,size(dims.mx,2),size(dims.sz,2));
%%
tic;
for i=1:size(dims.sx,2)
    ...
    rx=dims.rx;
    %%
    [p,rec]=solver(nz,nx,nt,dh,dt,sz,sx,s,rz,rx,v);
    true_rec(:,:,i)=rec;
    fprintf('\ngenerate_true_rec source=%f/%f \t t=%f s',i,size(dims.sz,2),toc);
end
save('true_rec.mat','true_rec');
%% plot p
figure(2)
...
%%
load('true_rec.mat');
for i=1:size(true_rec,3)
    figure(5)
    subplot(2,1,1)
    imagesc(true_rec(:,:,i),.1*[min(min(true_rec(:,:,i))),max(max(true_rec(:,:,i)))]);
    xlabel({['x*' num2str(dims.dh) '[m]']});
    ylabel({['t*' num2str(dims.dt) '[m]']});
    shg;
    pause(.5);
